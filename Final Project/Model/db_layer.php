...
function search($Productname){
    ...
    $sql="SELECT * FROM products WHERE Productname = '".$q."'";
...
function updateProduct($id, $data){
    $conn = db_conn();
    $selectQuery = "UPDATE products set Productname = ?, Buyingprice = ?, Sellingprice = ?, image=? where Id = ?";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
            $data['productname'], $data['buyingprice'], $data['sellingprice'],$data['image'], $id
        ]);
    }catch(PDOException $e){
        echo $e->getMessage();
    }
